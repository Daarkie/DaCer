    ...
    <main className="app-shell">
      <div className="top-bar">
        <nav className="tabs" aria-label="Main tabs">
          <button className={tab === "goals" ? "tab active" : "tab"} onClick={() => setTab("goals")}>Goals</button>
          <button
  ...
  return (
    <article className={active ? "item-card active" : "item-card"} onClick={onClick}>
      {isTask ? (
        ...
        <strong className="progress-number">{goalHasAnyTasks(item, goals, tasks) ? calculateGoalProgress(item, goals, tasks) + "%" : "N/A"}</strong>
      )}
...
function StatusPicker({ task, onChange }) {
  ...
  return (
    <div className="status-box" onClick={(event) => event.stopPropagation()}>
      <button className={`status-current ${task.status?.toLowerCase()}`} onClick={() => setOpen((value) => !value)}>
        {statusIcons[task.status] || statusIcons.NOT_STARTED}
      </button>
      {open && (
        <div className="status-menu">
          {statuses.map(([status, label]) => (
            <button key={status} onClick={() => { onChange(task, status); setOpen(false); }}>
      ...
      <dl className="detail-list">
        ...
        {isTask ? (
          <>
            ...
            <DetailLine label="Status" value={statusLabels[item.status] || item.status || "N/A"} />
            <DetailLine label="Started on" value={formatDate(item.started)} />
            <DetailLine label="Content" value={item.content || "N/A"} />
            ...
            <DetailLine label="Notes" value={item.notes || "N/A"} />
          </>
        ) : (
          <>
            ...
            <DetailLine label="Summary" value={item.summary || "N/A"} />
            <DetailLine label="Parent goal" value={parent?.name || "None"} />
            <DetailLine label="Sub-goals" value={subGoals.length ? subGoals.map((goal) => goal.name).join(", ") : "None"} />
            <DetailLine label="Sub-tasks" value={goalTasks.length ? goalTasks.map((task) => task.name).join(", ") : "None"} />
            <DetailLine label="Done" value={goalHasAnyTasks(item, goals, tasks) ? calculateGoalProgress(item, goals, tasks) + "%" : "N/A"} />
            <DetailLine label="Notes" value={item.notes || "N/A"} />
          </>
        )}
      </dl>
    ...
    <Modal title="New Goal" onClose={onClose}>
      <form className="entry-form" onSubmit={submit}>
        ...
        <Field label="Responsibility">
          <input maxLength={100} value={form.responsibility} onChange={(e) => setForm({ ...form, responsibility: e.target.value })} />
        </Field>
        <Field label="Summary">
          <textarea maxLength={350} value={form.summary} onChange={(e) => setForm({ ...form, summary: e.target.value })} />
        </Field>
        ...
        <Field label="Notes">
          <textarea maxLength={350} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
        </Field>
    ...
    <Modal title="New Task" onClose={onClose}>
      <form className="entry-form" onSubmit={submit}>
        ...
        <Field label="Parent goal" required>
          <select required value={form.parent_id} onChange={(e) => setForm({ ...form, parent_id: e.target.value })}>
            <option value="">Choose a goal</option>
            {goals.map((goal) => <option key={goal.id} value={goal.id}>{goal.name}</option>)}
          </select>
        </Field>
        <Field label="Content">
          <textarea maxLength={350} value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} />
        </Field>
        ...
        <Field label="Expected length" required>
          <div className="inline-input">
            <input type="number" min="0" value={form.length} onChange={(e) => setForm({ ...form, length: e.target.value })} />
            <span>days</span>
          </div>
        </Field>
        <Field label="Notes">
